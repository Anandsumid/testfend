...
function TableItems() {
  ...
  const handleDelete = (t)=>{
    setDelete(true)
    fetch(`http://18.140.72.65:3002/deletebook/id/${t}`,{
      method: "DELETE",
      headers: {
      ...
      .then((data)=>data.json())
      .then((data)=>setDelete(false))
  }

  useEffect(() => {
    fetch("http://18.140.72.65:3002/allbooks", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <div className="bg-white px-3 py-2 my-5">
      <Table striped hover>
        <thead>
          <tr>
            <th>#</th>
          ...
          {book.map((data, i) => {
            return (
              <tr key={i}>
                ...
                <td>{data.publish_date.slice(0, 10)}</td>
                <td><EditModal data={data}/></td>
                <td className="trash" onClick={()=> handleDelete(data._id)}>
                  {" "}
                  <svg
                    width="18"
                    height="20"
